...
public class RemoveTaskCommand : BaseCommand, IConversationCommand
{
    private readonly IToDoService _toDoService;

    public RemoveTaskCommand(ICommandService commandService, IToDoService toDoService) : base(commandService)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        await RemoveTask(taskGuid, user, botClient, update, cancellationToken);
    }
    ...
    private async Task RemoveTask(Guid taskId, ToDoUser? user, ITelegramBotClient botClient, Update update,
        CancellationToken cancellationToken = default)
    {
        var chatId = ToDoUtils.GetChatId(update);
        await _toDoService.Delete(taskId, cancellationToken);
        var markup = await GetCurrentTasksKeyboard(user, update, cancellationToken);
        await botClient.SendMessage(chatId, $"Удалена задача `{taskId}`", replyMarkup: markup, parseMode: ParseMode.Markdown, cancellationToken: cancellationToken);
    }
}